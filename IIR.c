...
void filter(IIR* X ,REAL* Signal, REAL* FilteredSignal) {
	...
	Output_Export(FilteredSignal);

}

void main() {
	//******************************DEFINICIJA ULAZNIH SIGNALA****************************************************//
	short int impuls[DULJINA_POLJA];
	int i;
	impuls[0] = 1;
	for (i = 1;i < DULJINA_POLJA;i++) {
		impuls[i] = 0;
	}
	short int step[DULJINA_POLJA] = { 1 };
	//************************************DEFINICIJA IIR FILTRA**************************************************//
	//karakteristike IIR filtra:
	//Tip: IIR butterworth pojasnopropusni filter
	//Frekvencija uzorkovanja: 40000 [Hz]
	//Sesti red
	//Donja granična frekvencija: 8400[Hz]
	//Gornja granična frekvencija: 13200[Hz]
	IIR* A;
	A->Akoef[7] = 0.0286353001709,
		0,
		-0.0859059005127,
		0,
		0.0859059005127,
